...
/**
 * Cria gráfico comparativo de diferentes métodos ACV
 * @param {HTMLElement} canvasElement - Elemento canvas
 * @param {Object} resultados - Resultados por método
 */
function criarGraficoComparativoMetodos(canvasElement, resultados) {
    const ctx = canvasElement.getContext('2d');
    
    const metodos = Object.keys(resultados);
    const valores = Object.values(resultados);
    
    // Define cores baseadas no método
    const cores = metodos.map(metodo => {
        switch(metodo) {
            case 'IPCC': return CORES_BIO.primaria;
            case 'CFF': return CORES_BIO.alerta;
            case 'Zero-Burden': return '#ff5722';
            default: return CORES_BIO.referencia;
        }
    });
    ...
    return new Chart(ctx, {
        type: 'bar',
        data: {
            labels: metodos.map(m => {
                if (m === 'IPCC') return 'ACV Padrão';
                if (m === 'CFF') return 'CFF';
                if (m === 'Zero-Burden') return 'Zero-Burden';
                return m;
            }),
            datasets: [{
                label: 'Intensidade de Carbono (gCO₂eq/MJ)',
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        afterLabel: function(context) {
                            const metodo = metodos[context.dataIndex];
                            if (metodo === 'CFF' || metodo === 'Zero-Burden') {
                                return '⚠️ Não reconhecido pelo RenovaBio';
                            }
                            return '';
                        }
                    }
...
/**
 * Atualiza dinamicamente a visualização quando dados mudam
 * @param {string} elementoId - ID do elemento canvas
 * @param {Object} dados - Novos dados para o gráfico
 */
function atualizarGrafico(elementoId, dados) {
    ...
    if (chart) {
        ...
        if (dados.labels) chart.data.labels = dados.labels;
        chart.update();
    }
}
...
/**
 * Função para adicionar animação de carregamento ao gráfico
 */
function mostrarCarregamentoGrafico(elementoId) {
    const container = document.getElementById(elementoId).parentElement;
    const loader = document.createElement('div');
    loader.className = 'grafico-loader';
    loader.innerHTML = `
 ...
 */
function removerCarregamentoGrafico(elementoId) {
    const container = document.getElementById(elementoId).parentElement;
    const loader = container.querySelector('.grafico-loader');
    if (loader) {
        loader.remove();
    }
}